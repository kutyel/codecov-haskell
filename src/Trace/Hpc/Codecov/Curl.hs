...
parseResponse r =
  ...
  where
    getField fieldName = fromJust $ mGetField fieldName
...
extractCoverage rBody =
  (++ "%") . show <$> (getField "coverage" :: Maybe Integer)
  where
...
readCoverageResult url printResponse = performWithRetry readAction
  where
    readAction = do
